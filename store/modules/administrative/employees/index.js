...
export const actions = {
  ...
  getAllListsFromApi({ commit }, payload) {
    ...
    commit('setListsValue', {
      key: 'employees',
      value: payload.employees.results
    })
  },

  async departmentIdChange({ state, commit, dispatch }, payload) {
    const { fields } = state
    ...
    const { data } = await this.$axios.$get(
      `/JobTitle/${fields.departmentId}/LoadEmployeeJobTitles`
    )
