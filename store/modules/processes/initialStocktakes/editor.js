...
export const actions = {
  ...
  calcTotal({ commit, state, dispatch }, payload) {
    ...
    const total =
      Number(singleUnitsQuantity) + Number(groupUnitsQuantity) * quantityInGroup
  ...
  async addToDB({ state, commit, dispatch }) {
    try {
      ...
      const schema = {
        ...
        fullSerialNumber: fields.fullSerialNumber,
      ...
      await this.$router.push(`/processes/initial-stocktakes/${data.id}`)
    } catch (error) {}
  },
